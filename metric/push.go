...
func StartMetricsPush(runner *task.Runner, cfg *MetricCfg) {
	if nil == cfg || cfg.DurationSync == 0 || len(cfg.Address) == 0 {
		logx.Info("metric: disable prometheus push client")
		return
	}
	...
	*client = *http.DefaultClient
	...
	logx.Info("metric: start prometheus push client")
	runner.RunCancelableTask(func(ctx context.Context) {
		...
		for {
			select {
			case <-ctx.Done():
				logx.Info("stop: prometheus push client stopped")
				ticker.Stop()
				return
			case <-ticker.C:
				err := doPush(cfg.Job, instanceGroupingKey(cfg.Instance), cfg.Address, prometheus.DefaultGatherer, "PUT")
				if err != nil {
					logx.Errorf("metric: could not push metrics to prometheus pushgateway: errors:\n%+v", err)
				}
			}
		}
	})
